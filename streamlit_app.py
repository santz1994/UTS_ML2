    ...
    def preprocess_input(input_data: pd.DataFrame, scaler, label_encoders, training_columns):
        ...
        for col, encoder in label_encoders.items():
            if col in input_data.columns:
                # Handle potential KeyError if a label is not in the encoder's classes
                try:
                    # Ensure the input value is in a list for transform
                    input_data[col] = encoder.transform(input_data[col].tolist())[0]
    ...
    # Add checks to ensure the encoder has classes before accessing
